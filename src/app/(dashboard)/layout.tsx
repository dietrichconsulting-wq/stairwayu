...
import { CounselorSidebar } from '@/components/dashboard/CounselorSidebar'
...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // Counselors bypass subscription gate entirely (free accounts)
  if (!isCounselor) {
  ...
  return (
    <div className="sidebar-layout">
      ...
      {isCounselor ? (
        <CounselorSidebar user={user} profile={profile} />
      ) : (
        <Sidebar user={user} profile={profile} subscription={subscription} />
      )}
